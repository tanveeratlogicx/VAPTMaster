...
- **Planned**: Fix "Sorry, you are not allowed" error on localhost by relaxing REST API permissions for administrators on local environments.
...
### Localhost Admin Notice & REST API (Bug Fix)
- **Problem**: Standard admins get a permission error in the React UI because the REST API strictly checks for Superadmin status.
- **Fix**: Update `VAPTM_REST::check_permission` and `vaptm_add_admin_menu` to allow *all* users with `manage_options` capability **IF** the environment is localhost.
- **Security**: Access *still* requires passing the **OTP Authentication** (emails still sent to Superadmin email).
- **Persistence**: Ensure `is_vaptm_localhost()` is globally available for these checks.
