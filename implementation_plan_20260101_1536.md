...
- **Planned**: Re-tighten Superadmin Dashboard access: Only the specified Superadmin can see the menu and link, even on localhost.
- **Planned**: Make OTP authentication mandatory for every browser session on localhost by switching from global options to user-specific transients.
- **Planned**: Fix the "Feature List" loading freeze in the React UI by isolating parallel API calls and adding "Loading..." indicators per component.
...
### Strict Superadmin Constraints (Revised v6)
- **Menu Registration**: Revert to strict registration. Only users whose username/email match `VAPTM_SUPERADMIN_USER` or `VAPTM_SUPERADMIN_EMAIL` will see the **VAPT Master Dashboard** submenu.
- **Admin Notice**: The localhost Admin Notice will *only* be shown to the Superadmin.
- **OTP Sessions**: Switch from `update_option` to `set_transient('vaptm_auth_' . $user_id, ...)`. This ensures the session is tied to the specific user and expires naturally, forcing re-authentication as requested ("even on a localhost, it should ask for the OTP").

### Feature List "Keeps Loading" (Bug Fix)
- **Investigation**: The current `Promise.all` in `admin.js` will fail the entire dashboard if one endpoint fails (e.g. data source JSON not found or REST permission change).
- **Fix**: 
    - Wrap individual `apiFetch` in `.catch()` within `Promise.all` to allow independent loading.
    - Add localized debug logs to see which specific endpoint is stalling.
    - Ensure the "Mark Implemented" button doesn't trigger a total reload if it's not needed.
