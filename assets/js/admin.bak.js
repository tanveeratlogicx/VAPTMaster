  ...
  const fetchData = (file = selectedFile) => {
    ...
    return Promise.all([fetchFeatures, fetchDomains, fetchDataFiles, fetchAssignees])
      .then(([featureData, domainData, files, assigneeData]) => {
        console.log('VAPT Master: Full data fetch complete', {
          featuresFound: featureData.length,
          domainsFound: domainData.length,
          filesFound: files.length,
          assigneesFound: assigneeData.length
        });

        const cleanedFiles = files.map(f => ({ ...f, label: f.label.replace(/_/g, ' ') }));
        setFeatures(featureData);
        setDomains(domainData);
        setDataFiles(cleanedFiles);
        setAssignees(assigneeData);
        setLoading(false);
        return { featureData, domainData, files, assigneeData };
      })
      .catch((err) => {
