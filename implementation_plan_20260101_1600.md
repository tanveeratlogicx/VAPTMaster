...
- **Planned**: Fix OTP & Menu Logic: Strictly restrict menu to Superadmin. Show Admin Notice for other admins *only* on localhost.
- **Planned**: Fix JSON Mapping: Map `name` to the Feature name correctly. Generate stable `key` from `name`.
- **Planned**: Add JSON Upload UI: Allow Superadmin to upload new feature lists.
- **Planned**: Ensure OTP is mandatory on localhost and always sent to `tanmalik786@gmail.com`.
